...
struct QuantizeAsymParam : public dmlc::Parameter<QuantizeAsymParam> {
  ...
  dmlc::optional<float> shift;

  DMLC_DECLARE_PARAMETER(QuantizeAsymParam) {
    DMLC_DECLARE_FIELD(scale)
    .set_default(dmlc::optional<float>())
    .describe("The scale for quantizing the FP32 data. If present, it will be used to quantize "
              "the FP32 data into uint8.");
    DMLC_DECLARE_FIELD(shift)
    .set_default(dmlc::optional<float>())
    .describe("The shift for quantizing the FP32 data. If present, it will be used to quantize "
              "the FP32 data into uint8.");
  }
};
  ...
  void Forward(const OpContext& ctx,
    ...
    if (input_data_dtype == mshadow::kUint8) {
      ...
      *outputs[2].dptr<float>() = 0;
      ...
      if (param.scale.has_value() && param.shift.has_value()) {
        Kernel<quantize_asymmetric, xpu>::Launch(
          s, outputs[0].Size(), outputs[0].dptr<uint8_t>(), outputs[1].dptr<float>(),
          outputs[2].dptr<float>(), inputs[0].dptr<float>(),
          param.scale.value(), param.shift.value());
      } else {
