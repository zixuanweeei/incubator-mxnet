    ...
    *outputs[2].data().dptr<float>() = 0;
    ...
    if (inputs[0].dtype() == mshadow::kInt8) {
      ...
      *outputs[2].data().dptr<float>() = 128;
      ...
      if (param_.scale.has_value() && param_.shift.has_value()) {
        scale = param_.scale.value();
        shift = param_.shift.value();
      } else {
        ...
        scale = mshadow::red::limits::MaxValue<uint8_t>() / (data_max - data_min);
        shift = mshadow::red::limits::MaxValue<uint8_t>() - data_max * scale;
      }
